...
def boundary_CL(x):
    return np.vstack( ( x[0] + np.ones_like(x[0]) * (0.2), x[1] + np.zeros_like(x[0]), np.zeros_like(x[0]) ))
...
alpha_l, alpha_d = 10, 10  # interior, boundary
# increasing alpha_d makes self-intersect

# IPTL = alpha_l / h_avg * inner( jump( grad(ul_h) ), jump( grad(wl_h)) ) \
#        - inner( dot( avg( grad(grad(ul_h)) ), n_F ) , jump( grad(wl_h) ) ) \
#        - inner( dot( avg( grad(grad(wl_h)) ), n_F ) , jump( grad(ul_h) ) )
...
GDBC_RL_bi = alpha_d / h * inner( dot( grad(ul_h), n_F), dot( grad(wl_h), n_F) ) \
             - inner( dot( grad(wl_h), n_F), dot( dot( grad(grad(ul_h)), n_F), n_F) )\
             - inner( dot( grad(ul_h), n_F), dot( dot( grad(grad(wl_h)), n_F), n_F) )

GDBC_RL_li = alpha_d / h * inner( GR_be_l, dot( grad(wl_h), n_F) ) \
             - inner( GR_be_l, dot( dot( grad(grad(wl_h)), n_F), n_F) )

GDBC_LL_bi = alpha_d / h * inner( dot( grad(ul_h), n_F), dot( grad(wl_h), n_F) ) \
             - inner( dot( grad(wl_h), n_F), dot( dot( grad(grad(ul_h)), n_F), n_F) )\
             - inner( dot( grad(ul_h), n_F), dot( dot( grad(grad(wl_h)), n_F), n_F)  )

GDBC_LL_li = alpha_d / h * inner( GL_be_l, dot(grad(wl_h), n_F)) \
             - inner( GL_be_l, dot( dot( grad(grad(wl_h)), n_F), n_F) )

f = fem.Constant(domain, default_scalar_type((0, 0, 0)))

a = inner( grad(grad(ul_h)), grad(grad(wl_h)) ) * dx + IPTL * dS + GDBC_RL_bi * ds(2) + GDBC_LL_bi * ds(1)
...
warped_l = grid_l.warp_by_vector("U0l", factor=1)
