...
# facets_T = mesh.locate_entities_boundary( domain, 0, lambda x: np.logical_and( np.isclose( x[0],0.5), np.isclose( x[1],1.0)) )
# dofs_T = locate_dofs_topological((V0, Q), 0, facets_T)
# u_T= Function(Q)
# u_T.interpolate(boundary_top)
# bc_T = dirichletbc(u_T, dofs_T, V0)
#
# facets_B = mesh.locate_entities_boundary( domain, 0, lambda x: np.logical_and( np.isclose( x[0],0.5), np.isclose( x[1],0.0)) )
