...
Vu = element("P", domain.basix_cell(),2, shape=(2,))
...
def boundary_CLx(x):
    return x[0] + np.ones_like(x[0]) * (0.2)
...
def GT_b(x):
    return np.vstack( ( np.zeros_like(x[0]), np.ones_like(x[0])*( cos(pi/4) )  ) )
...
# facets_CM = mesh.locate_entities( domain, 1, lambda x: np.logical_and( np.isclose( x[0], a/2), np.logical_and( x[1] <= b/2 + 1*b/NN, b/2 - 1*b/NN <= x[1] ) ) )
facets_CM = mesh.locate_entities( domain, 1, lambda x: np.isclose( x[0], a/2) )
dofs_CM = locate_dofs_topological((Q0, Qx), 1, facets_CM)
u_CM = Function(Qx)
u_CM.interpolate(boundary_CM)
bc_CM = dirichletbc(u_CM, dofs_CM, Q0)

facets_CN = mesh.locate_entities( domain, 1, lambda x: np.isclose( x[1], b/2) )
dofs_CN = locate_dofs_topological((Q1, Qy), 1, facets_CN)
u_CN = Function(Qy)
u_CN.interpolate(boundary_CN)
bc_CN = dirichletbc(u_CN, dofs_CN, Q1)
...
facets_CLx = mesh.locate_entities_boundary( domain, 1, lambda x: np.logical_and( np.isclose( x[0], 0.0), np.logical_and( x[1] <= b/2 + 1*b/NN, b/2 - 1*b/NN <= x[1] ) ) )
# facets_CLx = mesh.locate_entities_boundary( domain, 1, lambda x: np.isclose( x[0], 0.0) )
dofs_CLx = locate_dofs_topological((Q0, Qx), 1, facets_CLx)
u_CLx = Function(Qx)
...
facets_CRx = mesh.locate_entities_boundary( domain, 1, lambda x: np.logical_and( np.isclose( x[0], a), np.logical_and( x[1] <= b/2 + 1*b/NN, b/2 - 1*b/NN <= x[1] ) ) )
# facets_CRx = mesh.locate_entities_boundary( domain, 1, lambda x: np.isclose( x[0], a) )
dofs_CRx = locate_dofs_topological((Q0, Qx), 1, facets_CRx)
u_CRx = Function(Qx)
...
# facets_CLx = mesh.locate_entities_boundary( domain, 1, lambda x: np.isclose( x[0], 0.0) )
dofs_CLy = locate_dofs_topological((Q1, Qy), 1, facets_CLy)
u_CLy = Function(Qy)
...
# facets_CLx = mesh.locate_entities_boundary( domain, 1, lambda x: np.isclose( x[0], 0.0) )
dofs_CRy = locate_dofs_topological((Q1, Qy), 1, facets_CRy)
u_CRy = Function(Qy)
...
if bool(lr):
    ...
    # bcs = [bc_C, bc_CL, bc_CR, bc_CLx, bc_CRx]
    bcs = [bc_CL, bc_CR]
...
# c_1, c_2, d_1, d_2, d_3 = 1.0, 0.1, 0.1, 0.1, 1e-2
...
VL = functionspace(domain, element("P", domain.basix_cell(),2, shape=(2,)))
VL0 = VL.sub(0)
VLx, _ = VL0.collapse()
VL1 = VL.sub(1)
VLy, _ = VL1.collapse()
...
dofs_CM_l = locate_dofs_topological((VL0, VLx), 1, facets_CM)
ul_CM = Function(VLx)
ul_CM.interpolate(boundary_CM)
bc_CM_l = dirichletbc(ul_CM, dofs_CM_l, VL0)

dofs_CN_l = locate_dofs_topological((VL1, VLy), 1, facets_CN)
ul_CN = Function(VLy)
ul_CN.interpolate(boundary_CN)
bc_CN_l = dirichletbc(ul_CN, dofs_CN_l, VL1)

dofs_CRx_l = locate_dofs_topological((VL0, VLx), 1, facets_CRx)
ul_CRx = Function(VLx)
ul_CRx.interpolate(boundary_CRx)
bc_CRx_l = dirichletbc(ul_CRx, dofs_CRx_l, VL0)

dofs_CLx_l = locate_dofs_topological((VL0, VLx), 1, facets_CLx)
ul_CLx = Function(VLx)
ul_CLx.interpolate(boundary_CLx)
bc_CLx_l = dirichletbc(ul_CLx, dofs_CLx_l, VL0)

dofs_CRy_l = locate_dofs_topological((VL1, VLy), 1, facets_CRy)
ul_CRy = Function(VLy)
ul_CRy.interpolate(boundary_CRy)
bc_CRy_l = dirichletbc(ul_CRy, dofs_CRy_l, VL1)

dofs_CLy_l = locate_dofs_topological((VL1, VLy), 1, facets_CLy)
ul_CLy = Function(VLy)
...
if bool(lr):
    ...
    # bcs_l = [bc_C_l, bc_CR_l, bc_CL_l, bc_CRx_l, bc_CLx_l]
...
W = c_1 * inner( L, L ) +  d_1 * theta**2 + d_2 * inner( grad(theta), grad(theta) ) + d_3 * inner( G, G )
...
Vu2 = element("P", domain.basix_cell(),1, shape=(2,))
...
vals[:, :len(uh_i)] = uh_i.x.array.reshape((geometry.shape[0], len(uh_i))) - r_i.x.array.reshape((geometry.shape[0], len(r_i)))
grid["u"] = vals
...
# p.zoom_camera(1.2)
# p.show_axes()
# p.save_graphic("Miura_bowtie_10.eps")
p.show()
...
WF_ = replace(WF, {U: U})
v_reac = Function(MX)
virtual_work_form = form(action(WF_, v_reac))
...
# right
dofs_CR_R = locate_dofs_topological((V0, Q), 1, facets_CR)
u_CR_R = Function(Q)
u_CR_R.sub(0).interpolate(one)
bc_CR_R = dirichletbc(u_CR_R, dofs_CR_R, V0)
# u_CL.sub(0).interpolate(one)
bcs_R = [bc_CR_R]
fem.petsc.set_bc(v_reac.vector, bcs_R)
print("-" * 50)
print(f"Horizontal reaction Rx = {fem.assemble_scalar(virtual_work_form):.6f}")

# left
v_reac.vector.set(0.0)
dofs_CL_R = locate_dofs_topological(MX, 1, facets_CL)
u_CL_R = Function(MX)
u_CL_R.sub(0).sub(0).interpolate(one)
bc_CL_R = dirichletbc(u_CL_R, dofs_CL_R)
bcs_L = [bc_CL_R]
fem.set_bc(v_reac.vector, bcs_L)
print("-" * 50)
print(f"Horizontal reaction Lx = {fem.assemble_scalar(virtual_work_form):.6f}")
