...
while i < max_i:
    ...
    u_s_0 = sqrt(2.0)
    v_s_0 = sqrt(2.0)
    u_ts_0 = 2 * sin( ( acos( 1-cos( variable(theta_0) ) ) - variable(theta_0) )/2 )
    v_ts_0 = 2 * sin( ( acos( 1-cos( variable(theta_0) ) ) + variable(theta_0) )/2 )
    ...
    # bcs_ = [DirichletBC(Z.sub(0), boundary_CL, 1), DirichletBC(Z.sub(0), boundary_CR, 2),
    #         DirichletBC(Z.sub(0), boundary_CT, 3), DirichletBC(Z.sub(0), boundary_CB, 4),
    #         DirichletBC(Z.sub(1), Constant(theta_0), 1), DirichletBC(Z.sub(1), Constant(theta_0), 2),
    ...
    # u_t_p = cos( ( acos( 1-cos( theta ) ) - theta )/2 ) * ( -1 - sin(theta) / sqrt( - cos(theta) * ( cos(theta) -2 )  ) )
    ...
    dens = c_1 * inner(L, L) / J + d_1 * inner(H, H) + d_2 * inner(grad(theta), grad(theta)) + d_3 * theta ** 2    # test
    G = diff(dens, H)
    # G  = 2 * d_3 * H
    ...
    parameters = {'snes_max_it': 50, 'quadrature_degree': '2'}  # , "pc_type": "lu", "pc_factor_mat_solver_type": "mumps"}
    ...
    print(f"Initial angle {theta_0: .6f}: Reaction on the left = {res_l: .3e}")
