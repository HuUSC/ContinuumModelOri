...
sol_theta = odeint(actuation, theta0, t, args=(c_tau, c_kappa))
sol_omega_u, sol_omega_v = bendtwist(sol_theta, c_tau, c_kappa)
#print(sol_omega_v)
#sys.exit()
...
theta.vector()[:] = np.interp(coords, tv, sol_theta[:,0])

# plotting the result for theta
# final = VTKFile('theta.pvd')
# final.write(theta)

#Interpolate results from the ode computation in Cartesian coordinate
W = VectorFunctionSpace(mesh, 'CG', 1, dim=3)
PETSc.Sys.Print('Nb vec dof: %i' % W.dim())
omega_u = Function(W, name='omega_u')
omega_u.vector()[:,0] = np.interp(coords, tv, sol_omega_u[0,:])
omega_u.vector()[:,1] = np.interp(coords, tv, sol_omega_u[1,:])
omega_u.vector()[:,2] = np.interp(coords, tv, sol_omega_u[2,:])
omega_v = Function(W, name='omega_v')
omega_v.vector()[:,0] = np.interp(coords, tv, sol_omega_v[0,:])
omega_v.vector()[:,1] = np.interp(coords, tv, sol_omega_v[1,:])
omega_v.vector()[:,2] = np.interp(coords, tv, sol_omega_v[2,:])
...
# The following to be checked – 1/5/2025
...
sol_d = solve_ivp(ode_rot_d, [0, L], np.identity(3).flatten(), t_eval=tu, args=(omega_u, lu0)) # L/lu0
...
sol_l = solve_ivp(ode_rot_l, [0, H], np.identity(3).flatten(), t_eval=tv, args=(omega_v, lv0)) #H/lv0
...
# Do we need the top boundary?
sol_t = solve_ivp(ode_rot_t, [0, L], bc_l.at(0, H).flatten(), t_eval=tu, args=(omega_u, H, lu0)) #L/lu0
...
sol_r = solve_ivp(ode_rot_r, [0, H], bc_d.at(L, 0).flatten(), t_eval=tv, args=(omega_v, L, lv0)) #H/lv0
...
# bcs = [DirichletBC(Z, bc_d, 3), DirichletBC(Z, bc_l, 1), DirichletBC(Z, bc_r, 2)]
...
S = TrialFunction(Z)
T = TestFunction(Z)
a = ( inner( lv0 * S.dx(1) - dot(S, Omega_v), lv0 * T.dx(1) - dot(T, Omega_v)) * dx
      + inner( lu0 * S.dx(0) - dot(S, Omega_u), lu0 * T.dx(0) - dot(T, Omega_u)) * dx )
...
#Test
aux = Function(V)
aux.interpolate(det(dot(Reff.T, Reff)))
final = VTKFile('rot_indentity_test.pvd')
final.write(aux)
# sys.exit()
