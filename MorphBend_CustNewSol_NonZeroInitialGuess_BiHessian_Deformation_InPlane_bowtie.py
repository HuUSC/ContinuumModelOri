...
def lattice(angle):
    C1 = 183 - 120*sqrt(2) + 104*sqrt(3) - 72*sqrt(6)
    C2 = 2 * ( 110 - 77*sqrt(2) + 64*sqrt(3) - 43*sqrt(6) )
    C3 = -44 + 32*sqrt(2) - 28*sqrt(3) + 17*sqrt(6)
    C4 = -36 + 24*sqrt(2) - 20*sqrt(3) + 15*sqrt(6)
    C5 = 3 * ( 15 - 10*sqrt(2) + 8*sqrt(3) - 6*sqrt(6) )
    num_vd = C1 + C2 * cos(angle) - 3 * ( 1 + cos(angle) ) * sqrt( 2 + 4*sqrt(3) + 6*cos(angle) ) + \
             2 * cos(angle/2) * sqrt( 1 + 2*sqrt(3) + 3*cos(angle) ) * ( C3 + 3*sqrt(2)*cos(angle/2) + C4*cos(angle) ) \
             + C5 * cos(2*angle)

    den_vd = ( 5 + 2*sqrt(3) + ( 3 + 2*sqrt(3) ) * cos(angle) - 2 * ( 1 + sqrt(3) ) * cos(angle/2)
              * sqrt( 1 + 2*sqrt(3) + 3*cos(angle) ) ) * sqrt( C1 + C2*cos(angle) + 2 * cos(angle/2)
              * sqrt( 1 + 2*sqrt(3) + 3*cos(angle) ) * ( C3 + C4*cos(angle) ) + C5 * cos(2*angle) )

    ud_s, vd_s = - sqrt(3) * sin( angle/2 ), num_vd/den_vd
    return ud_s, vd_s


# mesh, trial and test space
# u_s, v_s = sqrt(3/2), sqrt( 2/5 * ( 5 + sqrt( 11 + 2*sqrt(3) - 4*sqrt( 9*sqrt(3) - 12 ) ) ) ) # phi = - 1 / 2.0 * Pi
# u_s, v_s = sqrt(3/2), sqrt( 2/5 * ( 5 - sqrt( 11 + 2*sqrt(3) + 4*sqrt( 9*sqrt(3) - 12 ) ) ) )  # phi = - 3 / 2.0 * Pi
u_s, v_s = sqrt( 3*( 2+sqrt(3) ) ) / 2, 2 / sqrt( 4 + sqrt(2) + sqrt(3) + sqrt(6) )  # phi = - 7 / 6.0 * Pi
# u_s, v_s = sqrt(3) / 2, sqrt( 2 - 2/13 * sqrt( 77 + 20*sqrt(3) + 24 * sqrt( -2 + 3*sqrt(3) ) ) )  # phi = - Pi -2/3
# * pi
...
c_1, c_2, d_1, d_2, d_3 = 1.0, 0.10, 1e-2, 1e-2, 1e-2
