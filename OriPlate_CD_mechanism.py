...
#get subspace of MX
V0 = MX.sub(0) # subspace for displacement
Q, _ = V0.collapse()
Q0 = MX.sub(0).sub(0) # subspace for x-component of displacement
Qx, _ = Q0.collapse()
Q1 = MX.sub(0).sub(1) # subspace for y-component of displacement
Qy, _ = Q1.collapse()
# V1 = MX.sub(1)
# R, _ = V1.collapse()
...
facets_R = mesh.locate_entities_boundary( domain, fdim, lambda x: np.isclose( x[0], a) )
dofs_R = locate_dofs_topological((Q0, Qx), fdim, facets_R)
u_R= Function(Qx)
u_R.interpolate(boundary_right)
bc_R = dirichletbc(u_R, dofs_R, Q0)

facets_L = mesh.locate_entities_boundary( domain, fdim, lambda x: np.isclose( x[0], 0.0) )
dofs_L = locate_dofs_topological((Q0, Qx), fdim, facets_L)
u_L = Function(Qx)
u_L.interpolate(boundary_left)
bc_L = dirichletbc(u_L, dofs_L, Q0)

facets_RC = mesh.locate_entities_boundary( domain, 0, lambda x: np.logical_and( np.isclose( x[0], a), np.isclose( x[1], b/2)) )
dofs_RC = locate_dofs_topological((Q1, Qy), 0, facets_RC)
u_RC = Function(Qy)
u_RC.interpolate(boundary_zero)
bc_RC = dirichletbc(u_RC, dofs_RC, Q1)

facets_LC = mesh.locate_entities_boundary( domain, 0, lambda x: np.logical_and( np.isclose( x[0],0.0), np.isclose( x[1], b/2)) )
dofs_LC = locate_dofs_topological((Q1, Qy), 0, facets_LC)
u_LC = Function(Qy)
u_LC.interpolate(boundary_zero)
bc_LC = dirichletbc(u_LC, dofs_LC, Q1)

# Collect Dirichlet boundary conditions
bcs = [bc_R, bc_L, bc_RC, bc_LC]
