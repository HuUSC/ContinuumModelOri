...
# domain, mesh
a, b = sqrt(3), 2.0
domain = mesh.create_rectangle(comm=MPI.COMM_WORLD, points=( (0.0, 0.0), (a, b) ), n=(20, 20),
...
def boundary_top_C(x):
    return np.ones_like(x[0])*(0.1)

def boundary_bottom_C(x):
    return np.ones_like(x[0])*(0.1)

fdim = domain.topology.dim - 1

facets_R = mesh.locate_entities_boundary( domain, 0, lambda x: np.logical_and( np.isclose( x[0], a), np.isclose( x[1], b/2)) )
dofs_R = locate_dofs_topological((V0, Q), 0, facets_R)
u_R= Function(Q)
u_R.interpolate(boundary_right)
bc_R = dirichletbc(u_R, dofs_R, V0)

facets_L = mesh.locate_entities_boundary( domain, 0, lambda x: np.logical_and( np.isclose( x[0],0.0), np.isclose( x[1], b/2)) )
...
# dofs_CTx = locate_dofs_topological((Q0, Qx), 0, facets_CT)
# u_CTx = Function(Qx)
# u_CTx.interpolate(boundary_zero)
# bc_CTx = dirichletbc(u_CTx, dofs_CTx, Q0)
#
...
u_CB = Function(Qz)
u_CB.interpolate(boundary_bottom_C)
bc_CB = dirichletbc(u_CB, dofs_CB, Q2)
#
# dofs_CBx = locate_dofs_topological((Q0, Qx), 0, facets_CB)
# u_CBx = Function(Qx)
# u_CBx.interpolate(boundary_zero)
# bc_CBx = dirichletbc(u_CBx, dofs_CBx, Q0)

# Collect Dirichlet boundary conditions
bcs = [bc_T, bc_B, bc_R, bc_L]
# bcs=[bc_R, bc_L, bc_CT, bc_CB, bc_CTx, bc_CBx]
